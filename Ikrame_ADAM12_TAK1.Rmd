...
```
##Définition:  sous-groupes d'expression de ADAM12 : 
...
## Test stats : AOV puis Tuckey
On compare toutes les conditions entre elles, le Tuckey est approprié suite à l'anova
...
```
## Niveau d'expression Adam12 dans les différents sous-types
```{r}
...
P
```
## Niveau d'expression Adam12 selon le stage 
```{r}
...
P2
```
## Niveau d'expression Adam12 selon le sous-type de cancer du sein
```{r}
...
## COnstruction des sous-ensemble tumeurs / NT
...
## Construction subset et calcul score TAK1 activé
...
## heatmap Colon
# A réécrire pour la nouvelle signature + utiliser ComplexHeatmap
```{r}
